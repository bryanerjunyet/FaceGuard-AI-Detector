...
All classes are self-contained with ZERO imports from the training/ package,
so they can be imported without triggering the training registry system.
...
class AdaIN(nn.Module):
    ...
    def forward(self, x, y):
        ...
        return ((x - x_mean.expand(size)) / x_std.expand(size)) \
...
class Conditional_UNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.up_last = nn.Upsample(scale_factor=4, mode="bilinear",
                                    align_corners=True)
...
class FairDetector(nn.Module):
    ...
    def __init__(self):
        ...
        xception_cfg = {"mode": "adjust_channel", "num_classes": 2,
                        "inc": 3, "dropout": False}
        ...
        self.block_spe = Conv2d1x1(self.encoder_feat_dim,
                                   self.half_fingerprint_dim,
                                   self.half_fingerprint_dim)
        self.block_sha = Conv2d1x1(self.encoder_feat_dim,
                                   self.half_fingerprint_dim,
                                   self.half_fingerprint_dim)
        self.block_fair = Conv2d1x1(self.encoder_feat_dim,
                                    self.half_fingerprint_dim,
                                    self.half_fingerprint_dim)
        self.block_fused = Conv2d1x1(self.encoder_feat_dim,
                                     self.half_fingerprint_dim,
                                     self.half_fingerprint_dim)
    ...
    def forward(self, data_dict: dict, inference=False) -> dict:
        ...
        return {
            "cls_ag": out_sha, "feat": sha_feat,
            "cls_spe": out_spe, "feat_spe": spe_feat,
            "cls_fair": out_fair, "feat_fair": fair_feat,
