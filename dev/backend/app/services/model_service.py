...
from .architectures import Xception, FairDetector
...
def _build_xception() -> nn.Module:
    return Xception({"mode": "original", "num_classes": 1,
                     "inc": 3, "dropout": False})
...
def _extract_pg_fdd(output: Any) -> float:
    """FairDetector returns dict with 'cls': (B, 1) fused logits."""
    logits = output["cls"]
    if logits.ndim > 1:
        logits = logits.squeeze(1)
    return torch.sigmoid(logits)[0].item()
...
# ── Registry ──────────────────────────────────────────────────
# To support a new model, add one entry here.
...
def _resolve_spec(model_name: str) -> ModelSpec:
    """Match a model_name to a registry entry (substring match)."""
...
class InferenceService:
    """Generic inference service — works with any registered model."""
    ...
    def _load_model(self) -> nn.Module:
        if not self.model_path.exists():
            raise FileNotFoundError(
                ...
                f"Place the .pth file in models/pretrained/."
            )
        ...
        if isinstance(checkpoint, dict):
            for wrapper_key in ("state_dict", "model_state_dict"):
                if wrapper_key in checkpoint and isinstance(
                        checkpoint[wrapper_key], dict):
                    checkpoint = checkpoint[wrapper_key]
                    break

        # Handle checkpoint keys that have a 'backbone.' prefix
        # (e.g. saved from XceptionDetector which wraps Xception).
        model_keys = set(model.state_dict().keys())
        ckpt_keys = set(checkpoint.keys())
        if not model_keys & ckpt_keys:
            prefix = next(iter(ckpt_keys)).split(".")[0] + "."
            stripped = {k[len(prefix):]: v for k, v in checkpoint.items()
